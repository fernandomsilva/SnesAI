...
int sfml_counter = 500;
bool sfml_activated = true;
int windowheight = 159 * 4/3;// = GFX.ScreenSize / (3 * GFX.RealPPL); // TODO: Find the correct value for window height and width
int windowwidth = 258;// = GFX.RealPPL / 2;
				   ...
				   int nCmdShow)
{
	...
	//sfml_gui.changeWindowSize(windowwidth, windowheight);
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    while (TRUE)
    {
            ...
            WaitForSingleObject (GUI.ClientSemaphore, 100) != WAIT_TIMEOUT)
        {
			...
			if(run_loop)
			{
				...
				//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				if (sfml_counter > 0)	sfml_counter--;
				if (sfml_activated && sfml_counter == 0) {
					windowheight = GFX.ScreenSize / (3/2 * GFX.RealPPL);
					windowwidth = GFX.RealPPL / 2;
					//sfml_gui.changeWindowSize(windowwidth, windowheight);
